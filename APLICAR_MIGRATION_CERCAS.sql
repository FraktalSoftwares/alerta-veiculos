-- =============================================
-- CERCAS VIRTUAIS - MIGRATION
-- =============================================
-- Copie e cole este arquivo completo no SQL Editor do Supabase Dashboard
-- Link: https://app.supabase.com/project/fjhozukksxzuazuykyhk/sql/new
...
CREATE INDEX IF NOT EXISTS idx_virtual_fences_equipment_id ON public.virtual_fences(equipment_id);
CREATE INDEX IF NOT EXISTS idx_virtual_fences_is_primary ON public.virtual_fences(is_primary) WHERE is_primary = true;
...
DROP TRIGGER IF EXISTS update_virtual_fences_updated_at ON public.virtual_fences;
...
DROP TRIGGER IF EXISTS ensure_single_primary_fence_trigger ON public.virtual_fences;
...
DROP POLICY IF EXISTS "Users can view fences of their equipment" ON public.virtual_fences;
DROP POLICY IF EXISTS "Users can create fences for their equipment" ON public.virtual_fences;
DROP POLICY IF EXISTS "Users can update fences of their equipment" ON public.virtual_fences;
DROP POLICY IF EXISTS "Users can delete fences of their equipment" ON public.virtual_fences;
...
USING (
    EXISTS (
        ...
        JOIN public.vehicles v ON v.id IN (
            SELECT vehicle_id FROM public.vehicle_equipment ve WHERE ve.equipment_id = e.id
        )
